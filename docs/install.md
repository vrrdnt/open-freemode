...
For selected properties, visit the **Alta Street Apartment Garage** or **Del Perro Apartment Garage** blip on foot. Confirm the menu explains that this first slice unlocks garage access without a housing interior, then purchase it using bank funds. The charge must happen once and ownership must survive a reconnect. Drive an owned vehicle into the property marker, store it, and retrieve it from that same garage. A character that has not purchased the property must be denied, and a vMenu or activity vehicle must still be rejected. Verify the marker sits on usable ground and that the configured exit remains clear before opening the server publicly.

For vendors, visit one of the four **Convenience Store** blips or either **AmmuNation Supplies** blip. Confirm only the focused six locations appear instead of ox_inventory's sample shop network. Buy a bandage, bulletproof vest or parachute and verify the advertised amount is charged from the bank once, the item appears in F2 inventory, and it remains after reconnecting. Fill the inventory and confirm an item that cannot fit is rejected without a charge. Purchases must also be rejected while the character is in an activity. vMenu continues to provide temporary freemode weapons, so these AmmuNation counters intentionally sell supplies rather than persistent firearms.
