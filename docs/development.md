...
Status: foundation implementation, 5 September 2026. This is a disposable tester package, not a public gameplay release. Classic onboarding, character appearance storage, money, vehicles, missions and the tutorial remain to be implemented. No starter cash or business portfolio is granted. Test accounts will be reset before the lasting public economy.
...
The database runner creates and removes its own MariaDB container and uses synthetic credentials, including delimiter characters. The image probe uses synthetic credentials and expects Cfx to reject the key; it does not attempt to bypass licensing or establish a game session. Neither test needs a production secret.
...
1. Build the image on the intended Linux amd64 node, or transfer the built image to that node using the operator's own image distribution process. The checked-in egg refers to `open-freemode:development`, a local tag, not a published registry image. Wings must be able to resolve it for both installation and runtime. For another reference, run `npm run egg -- YOUR_IMAGE_REFERENCE` before importing the generated JSON. Use immutable image digests for eventual releases.
...
6. Start using Pelican. The server remains unlisted and rejects players without `ofm.join`. `[ofm_db] Schema 1 ready.` is the egg's proposed readiness marker; it reports database/schema readiness, not proof that a client can load and play. Check that both custom resources start. The client joins using the game endpoint; there is no txAdmin setup step.
...
For restoration, restore matched files and a SQL dump to **separate** storage and a dedicated restore database, supply its new private credentials, and start with the original image. Check the original test account ID after reconnecting. A complete restore rehearsal is still outstanding; no recovery time or loss guarantee is established.
...
- Real MariaDB fixture: nine tests passed, covering concurrent profile creation, reconnection, schema locking, interrupted initialization, foreign/newer schema rejection and special-character credentials.
- Lua admission harness: authorization, database failure, duplicate identity, source-ID transition, spawn replay and reconnect decisions passed with mocked natives.

Still required: installed Panel/Wings versions and node architecture; real egg installation/ownership; a valid operator-supplied Cfx key; authenticated Enhanced resource load; private ConVar round-trip inside Cfx; client downloads and spawn; database outage during a real connection; live console/stop/restart; matched SQL/file restoration; a second independent installation. These remain the P0 acceptance gates. CI cannot substitute for them.
