...
The native terminal editor ignored LF-terminated console commands in a reproduced authenticated test. Wings appends LF to submitted commands. The launcher gives interactive Cfx a private terminal and translates LF/CR/CRLF command endings into terminal Enter. Non-interactive stdin retains its original behavior. A pipe-based adapter was rejected because Cfx could hang after a fatal startup error; both interactive and non-interactive invalid-key shutdown checks pass with the terminal adapter. Output and signal handling are retained. Enter `ofm_status` without a slash in Pelican. A `Schema 2 ready.` line establishes readiness even if an earlier resource filesystem warning is present; do not grant broad filesystem permissions solely to silence that warning.
